  ...
  return (
    <>
      <nav className="fixed top-4 right-4 z-50 md:top-6 md:left-1/2 md:right-auto md:-translate-x-1/2">
        ...
        <div className="hidden md:flex items-center gap-2 bg-background/70 backdrop-blur-xl border border-primary/20 rounded-full px-8 py-3 shadow-xl">
          {navItems.map((item) => (
            <button
              ...
              className="text-xs sm:text-sm font-medium text-foreground/80 hover:text-foreground px-3 py-2 transition-colors duration-200"
            >
              {item.label}
            </button>
          ))}
        ...
        {/* Mobile Navigation */}
        <div className="md:hidden flex items-center gap-2 bg-background/70 backdrop-blur-xl border border-primary/20 rounded-2xl px-4 py-2 shadow-xl">
          <button
          ...
          <div className="flex flex-col gap-2">
            {navItems.map((item) => (
              <button
                ...
                className="text-sm font-medium text-foreground/80 hover:text-foreground px-4 py-2 text-left hover:bg-primary/10 rounded-lg transition-colors duration-200"
              >
                {item.label}
              </button>
            ))}
